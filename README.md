...
1. channelid： 渠道号，默认渠道号（5410093632），默认下载的MM渠道包。
2. onIntent： 配置是否允许schema调起，schema调起不能区分MM版本类型，能提高调起概率。默认打开，默认值为true
3. useGuide： 是否允许展示用户引导弹窗，默认允许，默认值为1； 配置0，可关闭引导，关闭引导之后程序直接做MM下载或跳转应用宝。
4. wetchartmm： 配置微信下载MM的应用宝地址。
5. downloadmm： 调起失败是否下载MM，默认下载，如不允许下载，配置为0。
6. callOnlyVersion： 配置检查的MM版本，如果检查失败会使用schema调起，默认值（'MM|MMLite|MMOpen');
...
MM调起方法的统一处理流程：
  1. 首次激活，使用版本相关的socket激活方法（可通过callOnlyVersion配置感兴趣的版本），判断MM是否激活
   1.1 激活失败，按具体场景做二次激活处理
     1.1.1 微信： 弹窗引导用户跳转应用宝下载地址（可通过useGuide配置是否弹窗）
     1.1.2 非微信： 是否支持schema调起（通过onIntent配置）
